...
class Gear(SFC):
    ...
    # Добавленные переменные для отслеживания времени работы
    operating_time = POU.var(int(0), persistent=True)  # общее время работы в минутах
    time_threshold = POU.var(int(500), persistent=True)  # пороговое значение времени в минутах
    time_reached = POU.var(False)  # флаг достижения порога
    time_reset = POU.var(False)  # сигнал сброса счетчика
    _minute_counter = POU.var(int(0))  # счетчик секунд для минуты
    ...
    def _update_operating_time(self):
        ...
        if self.time_reset:
            ...
            self.log('Счетчик времени сброшен')
            return
            
        # Если оборудование работает, увеличиваем счетчик секунд
        if self.state == Gear.RUN and self.q and not self.fault:
            self._minute_counter += 1
            
            # Каждые 60 секунд (1 минута) добавляем 1 минуту к общему времени
            if self._minute_counter >= 60:
                self.operating_time += 1
                self._minute_counter = 0
                self.log(f'Накоплено времени: {self.operating_time} мин.')
                
                # Проверяем достижение порога
                if self.operating_time >= self.time_threshold and not self.time_reached:
                    self.time_reached = True
                    self.log(f'Достигнут порог времени: {self.operating_time} мин. из {self.time_threshold} мин.')
    ...
    def main(self):
        ...
        while T<5 and self._allowed():
            self._update_operating_time()  # обновляем время в процессе запуска
        ...
        if T<5 or self.fault:
            ...
            # Основной рабочий цикл с обновлением времени
            while self.q and self._allowed() and not self._lock:
                self._update_operating_time()  # постоянно обновляем время в рабочем режиме
                yield from self.pause(1000)  # пауза 1 секунда
